...
# Create a group with the same name as the user and add the current user to it
GROUP=$USER
...
# which was created during the MATE setup.
sudo mkdir -p /etc/skel/Documents/MATLAB/

# Configure MATLAB_ROOT directory and permissisons
sudo mkdir -p "${MATLAB_ROOT}"
sudo chown -R ${USER}:${USER} /home/${USER}/
sudo chmod -R 755 "${MATLAB_ROOT}"
sudo chmod g+s "${MATLAB_ROOT}"
...
sudo mkdir -p "/home/${USER}/.matlab/${RELEASE}"
sudo cp /var/tmp/config/matlab/matlab.prf "/home/${USER}/.matlab/${RELEASE}/"
sudo chown -R ${USER}:${GROUP} "/home/${USER}/.matlab"
