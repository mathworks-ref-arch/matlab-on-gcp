...
// Use this Packer configuration file to build VHD with R2024a MATLAB installed.
...
STARTUP_SCRIPTS         = [".env", "01_run-optional-user-command.sh", "10_setup-machine.sh", "20_setup-rdp.sh", "30_setup-matlab.sh", "40_warmup-matlab.sh"]
RUNTIME_SCRIPTS         = ["set-newuser-permissions.sh"]
BUILD_SCRIPTS           = ["install-startup-scripts.sh", "install-dependencies.sh", "install-ubuntu-desktop.sh", "install-mate.sh", "install-matlab-dependencies-ubuntu.sh", "install-matlab.sh", "setup-startup-accelerator.sh", "generate-toolbox-cache.sh", "cleanup.sh"]
